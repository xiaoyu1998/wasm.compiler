...
namespace wasm {
   ...
   private:
      ...
      public:
         ...
         {
            wasm::check(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
extern "C" {
void* malloc(size_t size) {
   return wasm::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size) {
   void* ptr = wasm::memory_heap.malloc(count*size);
   memset(ptr, 0, count*size);
   return ptr;
}

void* realloc(void* ptr, size_t size) {
   return wasm::memory_heap.realloc(ptr, size);
}

void free(void* ptr) {
   return wasm::memory_heap.free(ptr);
}
}
