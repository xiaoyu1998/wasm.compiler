...
void uniswap::add_liquidity(checksum256 exchange_hash,
    ...
    if (quantity.symbol == symbol(base_symbol) &&
        ...
        wasm::transaction inline_trx_liqs(get_self(),
                                          ...
                                          std::tuple(get_self(), owner, liquidity_minted,
                                                     get_self().to_string() + string(":add_liquidity")));
        inline_trx_liqs.send();

        wasm::transaction inline_trx_token(exchange.token_contract,
                                           name("transfer"),
                                           std::vector < permission > {{get_self(), name("wasmio.code")}},
                                           std::tuple(get_self(), owner, quantity - token_amount,
...
void uniswap::remove_liquidity(checksum256 exchange_hash,
    ...
    wasm::transaction
    inline_trx_liqs(wasm:name(base_bank),
            name("transfer"),
            std::vector < permission > {{get_self(), name("wasmio.code")}},
...
void
uniswap::base2tokenout(checksum256 exchange_hash, name to, asset max_base, asset tokens_bought, uint64_t deadline) {
    check(current_block_time() > deadline, "tx was expired");
...
void
uniswap::token2baseout(checksum256 exchange_hash, name to, asset base_bought, asset max_tokens, uint64_t deadline) {
    check(current_block_time() > deadline, "tx was expired");
    check(tokens_sold.amount > 0, "tokens_sold must > 0");
    check(min_base.amount > 0, "min_base must > 0");
