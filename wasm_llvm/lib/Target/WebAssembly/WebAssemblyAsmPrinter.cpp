...
void WebAssemblyAsmPrinter::EmitEndOfAsmFile(Module &M) {
  ...
  //xiaoyu
  bool has_wasm_import = false;
  bool has_wasm_abi    = false;
  bool has_wasm_action = false;
  bool has_wasm_notify = false;
  for (const auto &F : M) {
     if (F.hasFnAttribute("wasm_wasm_import"))
        has_wasm_import = true;
     if (F.hasFnAttribute("wasm_wasm_abi"))
        has_wasm_abi = true;
     if (F.hasFnAttribute("wasm_wasm_action"))
        has_wasm_action = true;
     if (F.hasFnAttribute("wasm_wasm_notify"))
        has_wasm_notify = true;
  }
  ...
  if (has_wasm_abi) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".wasm_abi";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("wasm_wasm_abi")) {
           StringRef abi = F.getFnAttribute("wasm_wasm_abi").getValueAsString();
  ...
  if (has_wasm_action) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".wasm_actions";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("wasm_wasm_action")) {
           StringRef action_name = F.getFnAttribute("wasm_wasm_action").getValueAsString();
  ...
  if (has_wasm_notify) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".wasm_notify";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("wasm_wasm_notify")) {
           StringRef name = F.getFnAttribute("wasm_wasm_notify").getValueAsString();
