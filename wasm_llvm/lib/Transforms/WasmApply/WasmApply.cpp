...
namespace {
  // WasmApply - Mutate the apply function as needed 
  struct WasmApplyPass : public FunctionPass {
    static char ID; 
    WasmApplyPass() : FunctionPass(ID) {}
    bool runOnFunction(Function &F) override {
       if (F.hasFnAttribute("wasm_wasm_entry") || F.getName().equals("apply")) {
...
char WasmApplyPass::ID = 0;
static RegisterPass<WasmApplyPass> X("apply_fixup", "Wasm Apply Fixups");

static void registerWasmApplyPass(const PassManagerBuilder&, legacy::PassManagerBase& PM) { PM.add(new WasmApplyPass()); }
static RegisterStandardPasses RegisterMyPass(PassManagerBuilder::EP_EarlyAsPossible, registerWasmApplyPass);
